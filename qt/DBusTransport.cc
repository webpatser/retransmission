...
public:
    ...
    [[nodiscard]] tr::interop::Reply present_window(std::string_view const activation_token) override
    {
        return tr::interop::present_window_via(
            ...
            [this](std::string_view const token) {
                return call_reply(MethodPresentWindowWithToken, QVariantList{} << Utils::qstringFromUtf8(token));
            },
    ...
    {
        return call_reply(MethodAddMetainfo, QVariantList{} << Utils::qstringFromUtf8(metainfo));
    }
    ...
    auto const peer = tr::interop::select_dbus_peer(
        ...
        [&bus](std::string_view const name) {
            ...
            return iface != nullptr ?
                QDBusReply<QString>{ iface->serviceOwner(Utils::qstringFromUtf8(name)) }.value().toStdString() :
                std::string{};
        });
