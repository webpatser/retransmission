...
{
    // main() checks this flag after exec() and skips its settings.json save;
    // the settings on disk are the holder's.
...
tr::interop::Reply Application::LocalInstance::present_window(std::string_view const activation_token)
{
    ...
    app_.window_->presentWindow(Utils::qstringFromUtf8(activation_token));
...
std::string Application::LocalInstance::config_dir()
{
    ...
    return tr::interop::canonical_config_dir(app_.config_dir_.toStdString());
}
