...
#include "../libtransmission/test-fixtures.h"
...
using InteropTest = tr::test::SandboxedTest;
...
TEST_F(InteropTest, contendedLaunchWaitsOutThePublisherThenDelegates)
{
    ...
    auto launcher = std::thread{ [&] { exit_code = launcher_coordinator->delegate(Intent::Present, {}, {}, 5s); } };
...
TEST_F(InteropTest, carriesARefusalThroughToTheExitCode)
{
    ...
    auto const provider = interop::MetainfoProvider{ [] { return std::vector<std::string>{ "alpha" }; } };
